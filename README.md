🚀 Desafio: 20 Dias de Código com Python
Repositório dedicado ao acompanhamento do meu progresso no desafio de 20 dias focados em lógica de programação e 
algoritmos utilizando a linguagem Python.

🎯 Objetivo
Aprimorar minhas habilidades de resolução de problemas e consolidar conceitos fundamentais e avançados da linguagem,
mantendo o hábito de codar diariamente.

🛠️ Tecnologias Utilizadas
Linguagem: Python 3.x
IDE: Online Python Compiler

📅 Progresso do Desafio

https://judge.beecrowd.com/
main -  Hello World!  
1 - Extremamente Básico
2 -Área do Círculo
3 - Soma Simples
4 - Produto Simples
5 - Média 1
6 - Média 2
7 - Diferença
8 - Salário
9 - Salário com bônus
10 - Calculo Simples
11- Esfera 
12 - Área
13 - O Maior
14 - Consumo



👤 Autor
Feito com ☕ por Andressa Alves 
Comunidade: Mais Meninas Tech 
Curso em Video pelo Yotube 
Professor Guanabara
Curso Python #07-Operadores Aritméticos
Curso Python #09 - Manipulando Texto
Primeiros Comando Python 3

Conecte-se comigo no LinkedIn : www.linkedin.com/in/andressa-alves-simão



